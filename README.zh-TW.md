...
接下來兩點的根是另一個：採集只是一條模型會遵守的指示，背後沒有任何東西知道
「說話的是誰」或「已經寫了多少」。
...
- **寫程式前應該讀完整條目，實際上並沒有。** `SKILL.md` 要求先打開
  ...
  *應已在 `21e2afc` 修正（Except 情境直接進索引行；`query.sh` 讓正確的讀法變便宜），但尚未驗證。*
- **讀 store 沒有工具，而且很吃 context。** 沒有查詢能力 —— 沒有「給我這個分類的條目」，
  ...
  *應已在 `21e2afc` 修正（`scripts/query.sh`），但尚未驗證。*
- **它分不出「轉向」來自人類還是另一個 AI。** 整套機制的前提是「人類出手轉向」——
  journal 自己的行文範式就是以「User……」開頭 —— 但 subagent 收到的 prompt 來自它的父代、
  session 之間的訊息來自另一個模型，而 hook 的輸入並不帶作者身分。
  於是跨 session 與跨 subagent 的往來（父對子、子對父、兄弟對兄弟）會像你親口說的一樣進到
  journal，甚至有機會進到批次提問裡，讓「收據」引用的其實是 AI 而不是你。
  在 store 開始記下說話者是誰之前，凡是你沒有親自主導的 session，它的收據都該當成可疑。
  *應已在 `d05757b` 修正（作者紀律加上分析的零號關卡 —— 但那是紀律，不是機制），但尚未驗證。*
- **記多少取決於跑在哪個介面，而不是取決於你實際轉向了多少。** 那份義務只是一行注入的文字、
  由模型自行遵守；背後沒有計數器、沒有速率限制、也沒有寫入時的去重，
  所以積極程度會隨執行環境而變 —— 同樣的工作量下，VS Code 擴充似乎比終端機 CLI 記得更多。
  而且這件事從 store 本身量不出來：來源標頭只帶 session、repo、暫存根與起始時間，
  沒有任何客戶端身分。所以這只能是一個印象，不是一個數字。
  *部分已在 `d05757b` 修正（來源標頭記下客戶端身分，讓它變得可量測；加上一事件一行的去重），
  但尚未驗證 —— 而且計數器與速率限制仍然沒有。*
