...
以下都是實際拿來跑真實工作時觀察到的。每一點現在都有對應的緩解措施，而每個緩解措施
也都有一個值得知道的洞。前三點的根仍然是同一個 —— store 是 Markdown，讀不讀、怎麼讀，
仍憑模型自己斟酌；接下來兩點的根也還在：採集是一條模型會遵守的指示，背後沒有任何
**機制**知道「說話的是誰」或「已經寫了多少」。

- **分析會誤解你的意思 —— 現在有關卡，但沒有消失。** 分析階段把 journal 的原始行轉成候選，
  過去它抓錯重點的頻率高到不能忽視：「而不是什麼」那一側被對調、只針對一個檔案講的話
  被一般化成一整類目標、推導出一個你根本不會給的理由。`DETECTION.md` 現在的 span 紀律
  要求候選的每個承重部分 —— 選中的那側、被捨棄的那側、範圍限定詞 —— 都必須對回收據裡的
  逐字引句；拿不出引句的部分標成 `inferred`，由提問來跟你確認而不是直接斷言，
  分不出方向的候選則把方向本身變成問題、絕不用猜的。洞在這裡：做檢查的還是寫出候選的
  同一個模型，所以收據與審閱關卡仍然是你的後盾 —— 只是不再是唯一的了。
- **`Except` 子句曾是整份 store 最少被讀到的地方。** 寫程式前本該讀完整條目，實際上
  模型靠注入的單行索引在做事、跳過檔案 —— 於是防止傾向誤觸的關鍵部分沒人讀。
  兩個改動打的是成本結構，不是紀律：索引行現在直接帶著每筆條目的 Except 情境
  （`[N except: …]`），而 `scripts/query.sh` 用幾行就能回傳一整個分類的
  topic、`observed-in` 與 Except。洞在這裡：Except 的**理由**仍然在檔案裡，
  而且連這條便宜的讀取路徑也沒有任何機制強制 —— usage log 依舊說不出它有沒有發生。
- **讀 store 曾經就是 `cat` 整個檔。** 現在有查詢了 ——
  `scripts/query.sh <root> [-c 分類] [-s slug] [-m 文字] [-f 欄位]` —— 帶欄位投影，
  查幾筆條目花的是幾行，而不是一頁頁的 frontmatter 與敘述。洞在這裡：要不要呼叫它
  仍然是模型的選擇；改變的是，正確的路不再是貴的那條。
- **人類與 agent 的轉向，是靠紀律分開的，不是靠機制。** 整套機制的前提是「人類出手轉向」，
  但 subagent 收到的 prompt 來自它的父代、session 之間的訊息來自另一個模型，
  而 hook 的輸入並不帶作者身分。journal 義務現在把「User」保留給人類，
  非人類的轉向記下實際作者（或標 `[author: unverified]`），分析階段的零號關卡
  則把這類行擋在候選與收據之外。洞在這裡：這是一條指示，而指示是可以不被遵守的 ——
  在 harness 把作者身分交給 hook 之前，凡是你沒有親自主導的 session，
  它的收據仍然值得多看一眼。
- **記多少仍然取決於跑在哪個介面。** 採集的積極程度隨執行環境而變 ——
  同樣的工作量下，VS Code 擴充似乎比終端機 CLI 記得更多。journal 的來源標頭
  現在會盡力記下客戶端身分（`CLAUDE_CODE_ENTRYPOINT`，退而求其次 `TERM_PROGRAM`），
  讓「似乎」變成一個可以跨 journal 數出來的數字，義務本身也加上了
  「一個事件一行」的去重。洞在這裡：義務背後仍然沒有計數器、沒有速率限制；
  變異數現在量得到了，但還沒有被管住。
