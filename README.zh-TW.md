...
資料夾）、開每個 session 的 journal、每回合注入索引、在段落點補 usage 帳，
...
| `stop.sh` | Stop | 這次回應改了檔案卻一筆 usage 都沒記時，攔下這次結束一次（每 session 每 30 分鐘最多一次），讓帳在段落被補上。條目有變動時在背景啟動一次雲端同步。它永遠不會啟動整併 |
...
4. **整併——只在你開口時。** 說「整理偏好」或 `consolidate`。確認過的區塊一次一筆
   併入 `prefer/`，而且要過審閱關卡；已結束 session 留下的 raw line 和候選也在
   同一趟被收進來。互相衝突的條目送進對抗式辯論，不會被無聲蓋掉。session 開頭
   會讓你看到有多少在等；你沒說，什麼都不會跑。
...
|---|---|---|
| A —— 捕捉 | 自動 | 記錄、分析、提問、套用 |
| B —— 整併 | 只接受明確指令 | 過審閱關卡，併入 `prefer/`；收進已結束 session 留下的東西 |
...
Mode B、D、E 絕不會由模型自己啟動。你下令之前，D 和 E 的參考檔連讀都不會被讀進
context。
...
- **Stop hook 在錯的時機強迫整併。** 在 `<fix-hash>` 之前，只要任何一份 journal 裡有
  已確認的區塊，Stop hook 就會攔下回應結尾，要模型「在這個自然段落點」跑 Mode B。
  但回應結束不等於任務邊界：它在使用者於另一個無關的 session 調查到一半時觸發，
  把人拉進審閱提問，而且積壓清完之前每個 session 都來一次。設計原本說的段落點，
  是使用者自己的完成訊號（「這樣就好」），hook 看不到。*已於 `<fix-hash>` 修正：
  Mode B 只在你下令時跑，session 開頭改成用一行看得見的提示告訴你積壓數量——
  日常使用尚未驗證。*
